...
namespace Exports.ExportTxtCustom
{
    ...
    **/
    ...
    class ExportTxtRilple : TxtToolBox
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(ExportTxtRilple));
        ...
        public override void LaunchExport()
        {
            ...
            var allowedColls = RepoManager.ColRepo.DbSet.Where(c => SelectedIds.Contains(c.Col_Id) && (c.Matricola_Col != null && c.Matricola_Col != "")).Select(c => new { c.Col_Id, c.Codice_Collaboratore, c.Cognome_Col, c.Nome_Col, c.Matricola_Col, c.Tab_Orari_Tipo_Id, c.Data_Disponibilita_Inizio_Col, c.Data_Disponibilita_Fine_Col }).ToList();
            ...
            foreach (var col in allowedColls)
            {
                ...
                string giustificativo5 = "";
                string oreGiustificativo5 = "";
                string giustificativo6 = "";
                string oreGiustificativo6 = "";
                List<string> giustificativiInseriti = new List<string>();

                DateTime startOfMonth = new DateTime(ExportDate.Year, ExportDate.Month, 1);
                string inizioMese = startOfMonth.ToString("ddMMyy");
                DateTime endOfMonth = ExportDate.EndOfMonth();
                string fineMese = endOfMonth.ToString("ddMMyy");

                exportData = exportData + codiceAzienda + "" + codiceFiliale + "" + codiceMatricola + "" + cognomeCol + "" + nomeCol + "" + inizioMese + "" + fineMese + "";

                var dataRegs = RepoManager.Reg_VRepo.GetAllQueryable().Where(r => r.Col_Id == col.Col_Id && r.Cant_Id != null && r.Data_Reg_AAAA_MM == formattedDate && (r.Registrazione_Tipo_Reg == (int)RegTypeEnum.None || r.Registrazione_Tipo_Reg == (int)RegTypeEnum.Duration || r.Registrazione_Tipo_Reg == (int)RegTypeEnum.RettTimeSheetManual) && r.Registrazione_Stato_Reg == (int)RegStateEnum.Ass).OrderBy(c => c.Data_Reg).GroupBy(r => r.Data_Reg);
