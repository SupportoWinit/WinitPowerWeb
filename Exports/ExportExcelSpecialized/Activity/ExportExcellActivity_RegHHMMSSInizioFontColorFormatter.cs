...
{
    public class ExportExcellActivity_RegHHMMSSInizioFontColorFormatter : ExportExcelTreeNodeAbstractFormatter
    {
        public override object Format(object currentValue, OfficeOpenXml.ExcelRange range, ExportExcelTreeNode currentNode)
        {
            ...
            if (nodeItem != null)
            {
                if (nodeItem.TipoModifica == (int)RegModifyTypeEnum.Exit_Modified ||
                    ...
                    fontColor = Business.Repository.RepoManager.ParamRepo.GetColorFromEnum(BusinessService.ColorModify((RegModifyTypeEnum)nodeItem.TipoModifica, RegEUEnum.Entry), false);
            }
            else
