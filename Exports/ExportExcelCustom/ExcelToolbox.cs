...
namespace Exports.ExportExcelCustom
{
    ...
    /// <summary>
    /// Classe astratta contenente tutti i metodi utilizzati per l'interfacciamento con excel degli
    /// export custom dell'applicativo.
    ...
    /// il metodo ExcelWorkbookDispose
    /// </summary>
    public abstract class ExcelToolbox<T> : IExportExcelCustom<T> where T : class
    {
        ...
        public virtual string ExcelModelFilePath
        {
            get; set;
        }

        public virtual DateTime ExportPeriod
        {
            get; set;
        }

        public virtual string[] GridFilter
        {
            get; set;
        }
        ...
        public virtual bool UseCalculationType
        {
            get; set;
        }

        public virtual bool UseHourType
        {
            get; set;
        }

        public virtual bool UseDurationTollerance
        {
            get; set;
        }

        public virtual bool UseEUTollerance
        {
            get; set;
        }

        public virtual bool UseExportDetail
        {
            get; set;
        }

        public virtual ExportRegVCalculationTypeEnum CalculationType
        {
            get; set;
        }

        public virtual ExportRegVHourTypeEnum HourType
        {
            get; set;
        }

        public virtual int DurationTollerance
        {
            get; set;
        }

        public virtual int EUTollerance
        {
            get; set;
        }

        public virtual ExcelModelSelectionTypeEnum ModelFirstEntity
        {
            get; set;
        }
        ...
        public abstract void LaunchExport(IQueryable<T> entitiesToExport);
        ...
        /// </summary>
        /// <param name="newFilePath">Il percorso file in cui salvare il foglio excel attualmente attivo.</param>
        /// <exception cref="System.ArgumentNullException">newFilePath</exception>
        /// <exception cref="System.InvalidOperationException">Excel workbook not initialized</exception>
        /// <exception cref="System.IO.DirectoryNotFoundException"></exception>
        public void ExcelWorkbookSaveToDisk(string newFilePath)
        {
            #region Convalida Input del metodo

            // convalida input del metodo: per essere salvato il percorso deve essere valorizzato, il foglio excel deve essere valorizzato,
            // la cartella di destinazione deve essere presente e un file non deve esistere nello stesso percorso
            ...
            if (!Directory.Exists(System.IO.Path.GetDirectoryName(newFilePath)))
                throw new DirectoryNotFoundException(BusinessService.GetLocalizedStringStrParam(PowerWebResources.ERR_PERCORSO_DIRECTORY_X_NON_VALIDO, System.IO.Path.GetDirectoryName(newFilePath)));

            if (File.Exists(newFilePath))
                throw new InvalidOperationException(BusinessService.GetLocalizedStringStrParam(PowerWebResources.ERR_NOME_FILE_X_GIA_PRESENTE, newFilePath));

            #endregion

            // salvataggio del file excel nel percorso indicato
            var workbookFileInfo = new FileInfo(newFilePath);
            ExcelWorkbook.SaveAs(workbookFileInfo);
        }

        /// <summary>
        /// Salva il foglio excel in lavorazione come risposta del browser utilizzando i parametri specificati.
        /// </summary>
        /// <param name="response">L'oggetto che rappresenta la risposta che riceverà il browser.</param>
        /// <param name="fileName">Il nome del file che dovrà avere il foglio excel nella risposta del browser.</param>
        /// <param name="inline">Se impostato a <c>true</c> allora la risposta sarà di tipo inline; se impostato a <c>false</c> la risposta sarà di tipo allegato.</param>
        /// <exception cref="System.InvalidOperationException">Excel workbook not initialized</exception>
        ...
        /// fileName
        /// </exception>
        public string ExcelWorkbookSaveToResponse(HttpResponse response, string fileName, bool inline)
        {
            string path = "";

            #region Convalida input del metodo

            // convalida input del metodo: affinchè il metodo possa esportare sulla risposta il file excel è necessario che il file excel sia valorizzato,
            // che la risposta non sia nulla e che il nome file non sia vuoto o nullo
            if (ExcelWorkbook == null)
                throw new InvalidOperationException("Excel workbook not initialized");
            ...
            using (var stream = new MemoryStream())
            {
                try
                {
                    ...
                    response.AddHeader("Content-Disposition", String.Format("{0}; filename={1}", (inline ? "Inline" : "Attachment"), fileName));
        ...
        public string ExcelWorkbookSaveToSession(string fileName)
        {
            using (var stream = new MemoryStream())
            {
                ...
                var fName = string.Format("{0}", fileName);
                PowerWebContext.SetToSession<MemoryStream>(fName, stream);
