...
namespace Exports.ExportExcelCustom.ExportSpecialized
{
    ...
    /// </summary>
    public class ExportDailyHour : ExcelToolbox<Reg_V>, IExportExcelCustom<Reg_V>
    {
        ...
        public override void LaunchExport(IQueryable<Reg_V> entitiesToExport)
        {
            ...
            List<Reg_V> regVs = RepoManager.Reg_VRepo.GetAllQueryable(r => r.Data_Reg.Value == today).OrderBy(r => r.Data_Ora_Fis_UTime).ToList();
            ExcelWorkbookGenerateNew(ExcelModelFilePath);
            TimeSpan totale = new TimeSpan(0, 0, 0);

            foreach (Reg_V reg in regVs) 
            {
                List<Cant> currentCant = RepoManager.CantRepo.GetAllQueryable(c => c.Cant_Id == reg.Cant_Id).ToList();
                Col currentCol = RepoManager.ColRepo.Single(c => c.Col_Id == reg.Col_Id);
                ...
                CellInsertValue(1, 2, rowIndex, currentCol.Nome_Col + " " + currentCol.Cognome_Col, ExcelInsertTypeEnum.Content);
                ...
                CellInsertValue(1, 3, rowIndex, reg.Data_Ora_Fig_ETime.ToString(), ExcelInsertTypeEnum.Content);
                ...
                if (reg.Data_Ora_Fig_UTime != null)
                {
                    TimeSpan from = new TimeSpan(reg.Data_Ora_Fig_ETime.Value.Hours, reg.Data_Ora_Fig_ETime.Value.Minutes, 0);
                    TimeSpan now = new TimeSpan(reg.Data_Ora_Fig_UTime.Value.Hours, reg.Data_Ora_Fig_UTime.Value.Minutes, 0);
                    TimeSpan differenza = now.Subtract(from);
                    string strFinale = differenza.Hours + ":" + differenza.Minutes;
                    if (differenza.Hours <= 10 && differenza.Minutes <= 0)
                    {
                        strFinale = "0" + differenza.Hours + ":0" + differenza.Minutes;
                    }
                    else if (differenza.Hours <= 10)
                    {
                        strFinale = "0" + differenza.Hours + ":" + differenza.Minutes;
                    }
                    else if (differenza.Minutes <= 10)
                    {
                        strFinale = differenza.Hours + ":" + differenza.Minutes;
                    }

                    CellInsertValue(1, 4, rowIndex, reg.Data_Ora_Fig_UTime.ToString(), ExcelInsertTypeEnum.Content);
                    ...
                    CellInsertValue(1, 5, rowIndex, strFinale, ExcelInsertTypeEnum.Content);
                ...
                else 
                {
                    ...
                    CellInsertValue(1, 4, rowIndex, now.ToString(), ExcelInsertTypeEnum.Content);
                    RangeSetBorders(1, 4, rowIndex, 4, rowIndex, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle);
                    RangeSetFontSize(1, 4, rowIndex, 4, rowIndex, 11);
                    RangeSetWrapText(1, 4, rowIndex, 4, rowIndex, true);
                    RangeSetBackgroundColor(1, 4, rowIndex, 4, rowIndex, Color.Yellow, fillStyle);

                    CellInsertValue(1, 5, rowIndex, strFinale, ExcelInsertTypeEnum.Content);
                    RangeSetBorders(1, 5, rowIndex, 5, rowIndex, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle);
                    RangeSetFontSize(1, 5, rowIndex, 5, rowIndex, 11);
                    RangeSetWrapText(1, 5, rowIndex, 5, rowIndex, true);
                    RangeSetBackgroundColor(1, 5, rowIndex, 5, rowIndex, Color.Yellow, fillStyle);
            ...
            CellInsertValue(1, 5, rowIndex, totale.TotalHours, ExcelInsertTypeEnum.Content);
            RangeSetBorders(1, 5, rowIndex, 5, rowIndex, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle, borderColor, borderStyle);
            RangeSetFontSize(1, 5, rowIndex, 5, rowIndex, 11);
            RangeSetWrapText(1, 5, rowIndex, 5, rowIndex, true);
