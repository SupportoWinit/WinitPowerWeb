...
namespace Exports.ExportExcelCustom.ExportSpecialized
{
    ...
    public class ExportSSSConInt : ExcelToolBox
    {
        ...
        private void WriteColTimesheet(Dictionary<string, List<TimesheetModuleItem>> cartellini)
        {
            ...
            foreach (var justification in cartellini["justification"])
            {
                ...
                if (justificationDec != "PART" && justificationDec != "FERM" && justificationDec != "OL" && justificationDec != "Ore Piano") { 
                    ...
                    string formulaToInsert = "";
                    if (CommonService.GetDatesFromPeriod(startMonth, endMonth).Count == 30)
                    {
                        formulaToInsert = "SUM(B" + rowIndex + ":AE" + rowIndex + ")";
                    }
                    else if(CommonService.GetDatesFromPeriod(startMonth, endMonth).Count == 31) {
                        formulaToInsert = "SUM(B" + rowIndex + ":AB" + rowIndex + ")";
                    }
                    ...
                    CellInsertValue(worksheetIndex, CommonService.GetDatesFromPeriod(startMonth, endMonth).Count + 2, rowIndex, totalHours, ExcelInsertTypeEnum.Formula);
