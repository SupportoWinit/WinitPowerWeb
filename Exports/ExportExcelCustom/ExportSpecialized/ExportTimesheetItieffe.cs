...
namespace Exports.ExportExcelCustom.ExportSpecialized
{
    ...
    public class ExportTimesheetItieffe : ExcelToolBox
    {

        #region Constants

        private static readonly ILog _log = LogManager.GetLogger(typeof(ExportTimesheetItieffe));
        ...
        private void WriteColTimesheet(Dictionary<string, List<TimesheetModuleItem>> cartellini)
        {
            ...
            int i = 0;
            int giorno = 0;
            //TimesheetModuleItem rep = cartellini.Where(c => c.Key == "REP");
            foreach (var justification in cartellini["justification"])
            {
                ...
                if (justificationDec != "PAUSA" && justificationDec != "REPERIBILITÀ") {
                    if (justificationDec == "ORE PIANO") {
                        piano = rowIndex;
                    }
                    ...
                    foreach (var day in CommonService.GetDatesFromPeriod(startMonth, endMonth))
                    {
                        ...
                        {
                            RangeSetBackgroundColor(worksheetIndex, columnIndex + day.Day, rowIndex, columnIndex + day.Day, rowIndex, Color.LightGray, fillStyle);
                        }
                    }
                    ...
                    rowIndex++;
                } else if (justificationDec == "REPERIBILITÀ") {
                    foreach (var day in CommonService.GetDatesFromPeriod(startMonth, endMonth))
                    {
                        var baseDuration = (double)justification["Day" + day.Day.ToString("00")];
                        if (baseDuration > 0 || i > 0) {
                            if (i < 7)
                            {
                                RangeSetBackgroundColor(worksheetIndex, columnIndex + day.Day, piano, columnIndex + day.Day, piano, Color.Yellow, fillStyle);
                                i++;
                            }
                            else {
                                i = 0;
