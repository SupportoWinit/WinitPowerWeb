...
namespace Exports.ExportExcelCustom.ExportSpecialized
{
    ...
    /// </summary>
    public class ExportExcelADP : ExcelToolbox<Reg_V>, IExportExcelCustom<Reg_V>
    {
        ...
        {
            /* LASCITO AI POSTERI */
            /* Se un giorno, in preda alla curiosità, vi imbatterete in questo file e ci chiederete cosa sia e a cosa serva questo export,
               vi prego, prima di continuare nella lettura del codice, di considerare che tale export è stato messo in piedi in circa 2 ore
               su richiesta urgente da parte del cliente Mosaico.
               Non giudicatemi per come è stato fatto e per l'impossibilità di utilizzarlo per altri clienti, sono consapevole di ciò
               e nonostante questo non avevo altra scelta per questioni di tempo.
               2016-11-30   Andrea Orler
            */
        ...
        {
            CellInsertValue(ExportWorkbookNumber, 1, _rowIndex, "5103", ExcelInsertTypeEnum.Content);
        ...
        {
            Col currentCol = RepoManager.ColRepo.First(c => c.Col_Id == colId);

            //Ritorna tutte le ore e i viaggi abbinati, senza tenere conto delle PAUSE
            var regs =  RepoManager.Reg_VRepo.Find(regv => regv.Col_Id == colId && regv.Data_Reg >= startPeriod && regv.Data_Reg <= endPeriod
            ...
            if(RepoManager.ParamRepo.GetCustomizationFromEnum(CustomizationEnum.SadCpExport) == 1)
            {
                Fil castanoFil = RepoManager.FilRepo.FirstOrDefault(c => c.Codice_Fil == "SAD CP");
                Cant sadCant = RepoManager.CantRepo.FirstOrDefault(c => c.Codice_Cantiere == "SAD CP00034");
                foreach (var reg in regs)
                {
                    if (sadCant != null  && reg.Cant_Id != null && reg.Cant_Id == sadCant.Cant_Id)
                    {
                        reg.Durata_Fig = 0;
                    }

                    if (reg.Registrazione_Tipo_Reg == (int)RegTypeEnum.Trip)
                    {
                        if (currentCol.Raggruppamento1_Col == castanoFil.Fil_Id.ToString())
                        {
                            reg.Durata_Fig = 0;
                        }
                    }
