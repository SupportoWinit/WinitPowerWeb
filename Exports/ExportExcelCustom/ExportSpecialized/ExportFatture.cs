...
{
    public class ExportFatture : ExcelToolbox<Reg_V>, IExportExcelCustom<Reg_V>
    {
                    ...
                    foreach (KeyValuePair<int, List<Reg_V>> groupedRegV in groupedRegVs)
                    {
                        ...
                        if (cant.First().Flag_NON_Esportare_Can == 0) 
                        {
                            ...
                            if (groupedRegV.Value.Any())
                            {
                                ...
                                foreach (Reg_V reg in groupedRegV.Value) 
                                {
                                    if (reg.Durata_Fig.HasValue)
                                    {
                                        totaleMensile += reg.Durata_Fig.Value;
                                    }
                                    else if(reg.Durata_Fis.HasValue)
                                    {
                                        totaleMensile += reg.Durata_Fis.Value;
                                    }
                                }
