...
if (!DevExpress.MOD_VIZ_GAUGES) {
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.gauges.__internals.BaseGauge = DX.viz.core.BaseWidget.inherit({
            ...
            _checkVisibility: function() {
                ...
                if (result) {
                    that._calculateSize();
                    result = that._width > 0 && that._height > 0 && that._element().is(':visible');
                    if (result)
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.gauges.Gauge = DX.viz.gauges.__internals.BaseGauge.inherit({
            ...
            _handleValueChanged: function(name, val) {
                var that = this;
                switch (name) {
                    case OPTION_VALUE:
        ...
        function selectObsoleteMode(that) {
            ...
            that._markers = [];
            that._rangeBars = [];
            that._handleValueChanged = function(name, val) {
                var that = this,
                    result = false;
                switch (name) {
                    case'needles':
                        setupValues(that, '_needleValues', val);
                        result = true;
                        break;
                    case'markers':
                        setupValues(that, '_markerValues', val);
                        result = true;
                        break;
                    case'rangeBars':
                        setupValues(that, '_rangeBarValues', val);
                        result = true;
                        break
                }
