...
if (!DevExpress.MOD_WIDGETS_BASE) {
        ...
        DX.registerComponent("dxRadioGroup", ui.CollectionContainerWidget.inherit({
            ...
            _valuesEqual: function(item1, item2) {
                ...
                if (dataSourceKey && !result) {
                    var item1Key = DX.utils.unwrapObservable(item1 && item1[dataSourceKey]);
                    var item2Key = DX.utils.unwrapObservable(item2 && item2[dataSourceKey]);
        ...
        DX.registerComponent("dxPopover", ui.dxPopup.inherit({
            ...
            _updateContentSize: function(containerLocation) {
                ...
                if (containerLocation.v.oversize > 0 && (positionAt === "top" || positionAt === "bottom")) {
                    var newContainerHeight = this._$container.height() - containerLocation.v.oversize,
                        newContentHeight = this._$content.height() - containerLocation.v.oversize;
                    this._$container.height(newContainerHeight);
                    this._$content.height(newContentHeight)
                }
            },
    ...
    (function($, DX, undefined) {
        ...
        var BaseView = DX.Class.inherit({
            ...
            views = {MonthView: BaseView.inherit({
                    ...
                    renderBody: function() {
                        ...
                        for (i = 0; i < this.weeks; ++i) {
                            ...
                            this.iterateDays(function(j) {
                                ...
                                $(cell).data("dxDateValue", cellDate);
                    ...
                    cellClickHandler: function(e) {
                        if (this.disabled)
                            return;
                        var cellDate = $(e.target).data("dxDateValue");
                    ...
                    keyDownHandler: function(options) {
                        ...
                        var newContouredDate = new Date(contouredDate.getFullYear(), contouredDate.getMonth(), contouredDate.getDate() + dayDifference);
                        utils.fixTimezoneGap(contouredDate, newContouredDate);
                        this.setContouredDate(newContouredDate)
                    },
        ...
        DX.registerComponent("dxCalendar", ui.dxEditor.inherit({
            ...
            _render: function() {
                ...
                var viewKeyDownProcessor = this._oldView ? this._oldView.detachKeyDownProcessor() : this._keyDownProcessor.attachChildProcessor();
            ...
            _renderMonthView: function() {
                ...
                }).off(CALENDAR_MONTHVIEW_CONTOUREDDATECHANGED_EVENT_NAME).on(CALENDAR_MONTHVIEW_CONTOUREDDATECHANGED_EVENT_NAME, function(event, date) {
                    if (date)
            ...
            _navigate: function(monthDifference) {
                if (this._canNavigate(monthDifference)) {
                    ...
                    newCurrentDate.setMonth(currentDate.getMonth() + monthDifference);
                    newCurrentDate.setDate(1);
                    utils.fixTimezoneGap(currentDate, newCurrentDate);
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxLookup", ui.dxEditor.inherit({
            ...
            _calcSelectedItem: function(callback) {
                ...
                this._loadSelectedItem(valueExpr, value).done(function(result) {
                    handleLoadSuccess(result)
                })
            },
            _loadSelectedItem: function(valueExpr, value) {
                var selectedItem = $.grep(this._dataSource.items(), $.proxy(function(item) {
                        return this._valueEquals(this._valueGetter(item), value)
                    }, this))[0];
                return DX.utils.isDefined(selectedItem) ? $.Deferred().resolve(selectedItem).promise() : this._loadSingle(valueExpr, value)
            },
            ...
            _handleDataSourceChanged: function() {
                var previousSelectedItem = this._selectedItem;
                this._calcSelectedItem($.proxy(function() {
                    this._selectedItem = this._selectedItem || previousSelectedItem;
                    this._setFieldText()
                }, this))
            },
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxAutocomplete", ui.dxDropDownEditor.inherit({
            ...
            _getItemRender: function() {
                if (!this._existTemplate())
                    return this.option("itemRender") || $.proxy(this._displayGetter, this)
            },
            _existTemplate: function() {
                var itemTemplate = this.option("itemTemplate");
                var templateOption = $.isFunction(itemTemplate) ? itemTemplate() : itemTemplate;
                var templateIsNode = typeof Node === "object" ? $(templateOption).get(0) instanceof Node : templateOption && typeof templateOption === "object" && typeof $(templateOption).get(0).nodeType === "number";
                return templateOption && (templateOption in this.option("_templates") || templateIsNode)
            },
            ...
            _optionChanged: function(name, value) {
                switch (name) {
                    case"items":
                        this._itemsToDataSource();
                        this._list.option("dataSource", this._dataSource);
                        break;
                    case"dataSource":
                        this._initDataSource();
                        this._list.option("dataSource", this._dataSource);
                    ...
                    case"itemTemplate":
                    case"itemRender":
                        this._list.option("itemTemplate", this._getTemplateByOption("itemTemplate"));
                        this._list.option("itemRender", this._getItemRender());
                        break;
                    case"searchMode":
                        this._validateSearchMode();
