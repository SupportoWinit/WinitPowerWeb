...
if (!DevExpress.MOD_VIZ_CORE) {
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.core.Legend = DX.Class.inherit({
            ...
            resetItem: function(id) {
                var index = this._getIndexById(id);
                if (index !== null)
                    this._applyMarkerOptions(this._markers[index], {fill: this._data[index].states.normal.fill})
            },
            ...
            _createMarker: function(data, group, markerIndex) {
                var that = this,
                    ...
                    normalStateColor = normalState.fill;
                marker = renderer.createRect(0, 0, size, size, 0, {
                    fill: normalStateColor,
                    opacity: normalState.opacity
                }).append(group);
                markersPatternsIds[markerIndex] = {
                    hoverPatternId: getPatternId(renderer, states, 'hover', normalStateColor),
                    selectedPatternId: getPatternId(renderer, states, 'selection', normalStateColor)
                };
                return marker
            },
                ...
                if (rowsColumns.autoEdit)
                    if (rows === 1) {
                        if (legendBox.width > placeholderWidth && columns > 1) {
                            columns = _floor(placeholderWidth / (maxBBox.width + options.columnItemSpacing)) || 1;
                        ...
                        if (legendBox.height > placeholderHeight && rows > 1) {
                            rows = _floor(placeholderHeight / (maxBBox.height + options.rowItemSpacing)) || 1;
    ...
    (function($, DX) {
        ...
        series.area = _extend({}, lineSeries, {
            ...
            _createLegendState: function(styleOptions, defaultColor) {
                var legendState = series.scatter._createLegendState.call(this, styleOptions, defaultColor);
    ...
    (function($, DX) {
        ...
        series.bubble = _extend({}, scatterSeries, {
            ...
            _parsePointStyle: function(style, defaultColor, defaultBorderColor) {
                var base = barSeries._parsePointStyle.call(this, style, defaultColor, defaultBorderColor);
