        ...
        $.extend(DX.viz.charts.Axis.prototype, {
            ...
            _drawConstantLine: function() {
                ...
                var getPos = function(lineValue) {
                        ...
                        if (!isDefinedUtils(value) || value < Math.min(canvasStart, canvasEnd) || value > Math.max(canvasStart, canvasEnd))
                            return {};
                        return {
                                value: value,
    ...
    (function($, DX, undefined) {
        ...
        var populateAxesInfo = function(axes) {
                return $.map(axes, function(axis) {
                        ...
                        if (tickValues && tickValues.length > 0 && utils.isNumber(tickValues[0]) && options.type !== 'discrete') {
                            ...
                            if (minValue === maxValue && isDefinedUtils(synchronizedValue)) {
                                minValue = tickValues[0] - 1;
                                maxValue = tickValues[0] + 1;
                                tickInterval = 1
                            }
                            axisInfo = {
                                axis: axis,
        ...
        var correctAfterSynchronize = function(axesInfo) {
                ...
                _each(axesInfo, function(i, info) {
                    ...
                    else if (!isDefinedUtils(correctValue) && !isDefinedUtils(info.synchronizedValue)) {
                        correctValue = math.abs((info.maxValue - info.minValue) / (info.tickValues[mathFloor(info.tickValues.length / 2)] || info.maxValue));
                        validAxisInfo = info
                    }
                });
                ...
                _each(invalidAxisInfo, function(i, info) {
                    var firstTick = info.tickValues[0],
                        ...
                        centralTick = tickValues[mathFloor(tickValues.length / 2)];
                    if (firstTick > 0) {
