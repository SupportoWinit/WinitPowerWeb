<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="RegV_ErrModule.ascx.cs" Inherits="PowerWeb.Modules.RegVErrModule" %>
...
<script type="text/javascript">
    ...
    function gvRegVErrEdit_OnEndCallback(s, e) {
    ...
    function applyPreFilter() {
        ...
        var errList = ASPxClientListBox.Cast(ErrorsListBox);
    ...
    function OnListBoxSelectionChanged(listBox, args) {
        if (ErrorsListBox.GetItem(0).selected) {
            ErrorsListBox.UnselectAll();
            ErrorsListBox.SetSelectedIndex(0);
        }
    ...
    function UpdateSelectAllItemState() {
        IsAllSelected() ? ErrorsListBox.SelectIndices([0]) : ErrorsListBox.UnselectIndices([0]);
    }
    function IsAllSelected() {
        var selectedDataItemCount = ErrorsListBox.GetItemCount() - (ErrorsListBox.GetItem(0).selected ? 0 : 1);
        return ErrorsListBox.GetSelectedItems().length == selectedDataItemCount;
    }
    function UpdateText() {
        var selectedItems = ErrorsListBox.GetSelectedItems();
        ErrorsComboBox.SetText(GetSelectedItemsText(selectedItems));
    }
    function SynchronizeListBoxValues(dropDown, args) {
        ErrorsListBox.UnselectAll();
        var texts = dropDown.GetText().split(textSeparator);
        var values = GetValuesByTexts(texts);
        ErrorsListBox.SelectValues(values);
    ...
    function gvRegVErrEdit_BatchEditStartEditing(s, e) {
        try{
...
<div style="clear: both; float: none; border: 1px solid darkgrey; padding: 5px">
    <table>
        <tr>
            <td>
                <dx:ASPxLabel ID="StateLabel" runat="server" Text="Errori da ricercare: " Font-Bold="true" />
            </td>
            <td>
                <dx:ASPxDropDownEdit ClientInstanceName="ErrorsComboBox" ID="ErrorsComboBox" Width="380px" runat="server" AnimationType="None" ReadOnly="True">
                    <DropDownWindowTemplate>
                        <dx:ASPxListBox Width="100%" ID="ErrorsListBox" ClientInstanceName="ErrorsListBox" SelectionMode="CheckColumn"
                            runat="server">
                            <Items>
                                <dx:ListEditItem Text="99 - Seleziona tutto" Value="99" Selected="True" />
                                <dx:ListEditItem Text="0 - Ore Singole" Value="0" />
                                <dx:ListEditItem Text="1 - Attività Singole" Value="1" />
                                <dx:ListEditItem Text="2 - Durata inferiore al minimo" Value="2" />
                                <dx:ListEditItem Text="4 - Durata superiore al massimo" Value="4" />
                                <dx:ListEditItem Text="9 - Sovrapposte" Value="8" />
                            </Items>
        ...
        <PanelCollection>
            <dx:PanelContent ID="filterPanelContent" runat="server" SupportsDisabledAttribute="True">
                ...
                <div style="float: left; width: 100%;">
                    <dx:ASPxGridView ID="gvRegVErrEdit" runat="server" AutoGenerateColumns="False" Width="100%"
                        OnCustomCallback="gvRegVErrEdit_CustomCallback" OnDataBinding="gvRegVErrEdit_DataBinding" OnAfterPerformCallback="gvRegVErrEdit_AfterPerformCallback"
                        OnInit="gvRegVErrEdit_OnInit" OnRowUpdating="gvRegVErrEdit_OnRowUpdating" OnRowDeleting="gvRegVErrEdit_OnRowDeleting"
                        OnRowInserting="gvRegVErrEdit_OnRowInserting" OnBatchUpdate="gvRegVErrEdit_BatchUpdate" ClientSideEvents-BatchEditConfirmShowing="Grid_BatchEditConfirmShowing"
                        OnCommandButtonInitialize="gvRegVErrEdit_OnCommandButtonInitialize"
                        ClientSideEvents-EndCallback="gvRegVErrEdit_OnEndCallback">

                        <ClientSideEvents BatchEditConfirmShowing="Grid_BatchEditConfirmShowing" EndCallback="gvRegVErrEdit_OnEndCallback" BatchEditStartEditing="gvRegVErrEdit_BatchEditStartEditing"></ClientSideEvents>
