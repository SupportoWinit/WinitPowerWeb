...
{
    public class ColTimesheetCellUpdate : ITimesheetUpdate
    {
        ...
        public void UpdateRettifica(TimesheetCellUpdate update)
        {
            ...
            RepoManager.Reg_VRepo.DbSet.Where(x =>
                                              x.Col_Id == update.BaseEntity_Id &&
                                              x.Cant_Id == update.GroupedEntity_Id &&
        ...
        void GenerateEqualizeReg(TimesheetCellUpdate update, TimeSpan totalDayDuration)
        {
            ...
            var newCorrectionReg = RepoManager.RegRepo.GenerateCorrectionReg(update.BaseEntity_Id, update.Date, cte, correctionRegDuration, update.GroupedEntity_Id);
        ...
        void GenerateNewJustification(TimesheetCellUpdate update)
        {
            ...
            var newDurationReg = RepoManager.RegRepo.GenerateDurationReg(update.BaseEntity_Id, update.Date, cte, (update.Time).Duration(), update.GroupedEntity_Id, _currentMotivation.Tab_Decod_Id);
        ...
        {
            var cte = (update.Time > TimeSpan.Zero) ? CorrectionTypeEnum.CorrectionPlus : CorrectionTypeEnum.CorrectionMinus;
            var newDurationReg = RepoManager.RegRepo.GenerateManualRett(update.BaseEntity_Id, update.Date, cte, (update.Time).Duration(), update.GroupedEntity_Id);
